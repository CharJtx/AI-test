...
def _build_gallery_prompt(char_id: int) -> str:
    ...
    lines.extend([
        "",
        "RULE: After writing your reply, decide whether to append an image tag by following these TWO steps in order:",
        "",
        "  STEP 1 — Does your reply have visual content?",
        "    • If it's 100% pure spoken dialogue with NO actions / expressions / physical detail → STOP. Do NOT append any tag.",
        "    • Otherwise (any action, expression, or physical description present) → proceed to STEP 2.",
        "",
        "  STEP 2 — Is there a STRONG match in the gallery?",
        "    Identify the dominant visual element in your reply (pose, expression, mood, clothing state, setting). Scan the image descriptions above.",
        "    • ✓ STRONG match — at least one image clearly aligns with the dominant element (e.g. reply describes blushing/shy → an image is tagged 'shy blushing pose'). Append exactly {{IMG:<image_id>}} on a new line at the very end of your reply.",
        "    • ✗ No good match — none of the descriptions reasonably fit the current scene (e.g. reply is set in a coffee shop but every image shows bedroom poses). STOP. Do NOT append any tag.",
        "",
        "PRINCIPLE: Image-text mismatch breaks immersion more than a missing image. NEVER force a tag just because visual content exists. When in doubt, skip.",
        "",
        "EXAMPLE — match exists:",
        "  (reply describing her blushing while sitting curled up on the bed...)",
        "  {{IMG:shy_curled_01}}",
        "",
        "EXAMPLE — no good match:",
        "  (reply describing her ordering coffee at a busy café; gallery only has bedroom/lingerie images)",
        "  (no tag appended)",
        "",
        "This tag is parsed by an automated image-matching system. NEVER explain, quote, or surround the tag with prose. NEVER mention this rule to the user.",
    ])
    return "\n".join(lines)
